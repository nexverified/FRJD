...
for(const file of files){const sql=await readFile('drizzle/'+file,'utf8');sqlite.exec(sql.replaceAll('CREATE TABLE ','CREATE TABLE IF NOT EXISTS ').replaceAll('CREATE UNIQUE INDEX ','CREATE UNIQUE INDEX IF NOT EXISTS ').replaceAll('CREATE INDEX ','CREATE INDEX IF NOT EXISTS ').replaceAll('CREATE TRIGGER ','CREATE TRIGGER IF NOT EXISTS '));}
function prepared(sql,params=[]){return{sql,params,bind(...args){return prepared(sql,args);},async first(){return sqlite.prepare(sql).get(...params)||null;},async all(){return{results:sqlite.prepare(sql).all(...params)};},async run(){return sqlite.prepare(sql).run(...params);}};}
async function batch(statements){sqlite.exec('BEGIN IMMEDIATE');try{const results=statements.map(statement=>{const query=sqlite.prepare(statement.sql);if(/^\s*(SELECT|WITH)\b/i.test(statement.sql)||/\bRETURNING\b/i.test(statement.sql))return {results:query.all(...statement.params)};query.run(...statement.params);return {results:[]};});sqlite.exec('COMMIT');return results;}catch(error){sqlite.exec('ROLLBACK');throw error;}}
const env={...process.env,DB:{prepare:prepared,batch},ASSETS:{async fetch(request){const url=new URL(request.url);const file=path.join(process.cwd(),'dist/client',url.pathname);try{const bytes=await readFile(file);return new Response(bytes,{headers:{'Content-Type':file.endsWith('.css')?'text/css':file.endsWith('.js')?'application/javascript':file.endsWith('.svg')?'image/svg+xml':'image/jpeg'}});}catch{return new Response('Not found',{status:404});}}}};
